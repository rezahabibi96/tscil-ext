...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train the continual learning agent on task sequence"
    )
    ...
    parser.add_argument(
        ...
        choices=[
            ...
            "Herding",
            "CLOPS",
            "ER_Sub"  # ER_Sub is only applicable to DSA
            "Mnemonics",
            "Inversion",
            "GR",
            "FastICARL",
    ...
    # Backbone
    parser.add_argument(
        "--encoder", dest="encoder", default="CNN", type=str, choices=["CNN", "TST"]
    )
    ...
    parser.add_argument(
        "--criterion", dest="criterion", default="CE", type=str, choices=["CE", "BCE"]
    )

    # Main classification loss and activation of head
    parser.add_argument(
        "--ncm_classifier",
        dest="ncm_classifier",
        type=boolean_string,
    ...
    )  # ZScore is only applicable for Offline

    # General params
    parser.add_argument(
        "--runs", dest="runs", default=1, type=int, help="Number of runs"
    )
    parser.add_argument(
        "--epochs", dest="epochs", default=100, type=int, help="Number of epochs"
    )
    parser.add_argument(
    ...
    # #################### Nuisance variables  ####################
    parser.add_argument(
        "--tune", type=boolean_string, default=False, help="flag of tuning"
    )
    parser.add_argument(
        "--debug", type=boolean_string, default=True
    )  # save the results in a 'debug' folder
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.05,
        help="Percentage of mem_budget/# train data",
    )
    parser.add_argument(
        "--buffer_tracker", type=boolean_string, default=False
    )  # Never be ture
    ...
    parser.add_argument(
        ...
        type=str,
        default="part",
        choices=["balanced", "part"],
        help="2 variant of ER to utilize subject labels.",
    )

    # KD: LwF / DT2W
    parser.add_argument(
        "--teacher_eval", type=boolean_string, default=False
    )  # As Online CL survey and Avalanche
    ...
    parser.add_argument(
        "--k_freq", type=int, default=-1
    )  # All freq if == -1, no loss_freq if == 0
