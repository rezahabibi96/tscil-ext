...
def offline_train_eval(task_stream, run, args):
    ...
    train_loader = Dataloader_from_numpy(
        x_train, y_train.squeeze(), batch_size=args.batch_size, shuffle=True
    )
    del x_train, y_train

    val_loader = Dataloader_from_numpy(
        x_val, y_val.squeeze(), batch_size=args.batch_size, shuffle=False
    )
    del x_val, y_val

    test_loader = Dataloader_from_numpy(
        x_test, y_test.squeeze(), batch_size=args.batch_size, shuffle=False
    )
    del x_test, y_test
    ...
    early_stopping = EarlyStopping(
        path=ckpt_path, patience=args.patience, mode="min", verbose=False
    )
    ...
    for epoch in range(args.epochs):
        epoch_loss, epoch_acc = epoch_run(
            model, train_loader, opt, scheduler, criterion, epoch, args, train=True
        )
        epoch_loss_val, epoch_acc_val = epoch_run(
            model, val_loader, opt, scheduler, criterion, epoch, args, train=False
        )
    ...
    val_loss, val_acc = epoch_run(
        model, val_loader, opt, scheduler, criterion, None, args, train=False
    )

    # Eval on test set
    if args.cf_matrix and args.tune is False:
        test_loss, test_acc, y_pred, y_true = test_epoch_for_cf_matrix(
        ...
        plot_confusion_matrix(
            ...
            y_pred,
            path=cf_matrix_path,
            classes=np.arange(task_stream.n_classes),
        )
    else:
        test_loss, test_acc = epoch_run(
            model, test_loader, opt, scheduler, criterion, None, args, train=False
        )

    if args.verbose:
        print(
            "Run {} val loss {}, val accuracy: {:.2f} ; test loss {}, test accuracy: {:.2f}".format(
                run, val_loss, val_acc, test_loss, test_acc
            )
        )
    ...
    for run in range(args.runs):
        ...
        print(
            "\n ######## {} tasks, Run {}, cls_order :{} ########".format(
                args.stream_split, run, cls_order
            )
        )
