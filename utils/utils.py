...
class EarlyStopping:
    ...
    def __init__(
        self, path, patience=5, mode="max", verbose=False, delta=0, trace_func=print
    ):
        """
        Args:
    ...
    def save_checkpoint(self, val_metric, model):
        ...
        if self.verbose:
            self.trace_func(
                f"Validation loss decreased ({self.val_metric_best:.6f} --> {val_metric:.6f}). Saving model ..."
            )
...
def mini_batch_deep_features(model, total_x, num):
    ...
    with torch.no_grad():
        ...
        for i in range(num_itr):
            ...
            deep_features_list.append(
                batch_deep_features_.reshape((batch_x.size(0), -1))
            )
            sid = eid
