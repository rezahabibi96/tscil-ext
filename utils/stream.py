...
from utils.data import (
    extract_samples_according_to_labels,
    extract_samples_according_to_labels_with_sub,
)
...
class IncrementalTaskStream(object):
    def __init__(self, data, scenario, cls_order, split):
        ...
        self.order_list = cls_order[
            start * self.n_class_per_task : end * self.n_class_per_task
        ]

        print(
            "Create {} stream : {} tasks,  classes order {} ".format(
                self.split, self.n_tasks, self.order_list
            )
        )
    ...
    def setup(self, cut=0.9, load_subject=False):
        ...
        for t in range(self.n_tasks):
            ...
            if load_subject:
                x_train_t, y_train_t, sub_train_t = (
                    extract_samples_according_to_labels_with_sub(
                        x_train, y_train, sub_train, classes_in_task_t
                    )
                )
            else:
                x_train_t, y_train_t = extract_samples_according_to_labels(
                    x_train, y_train, classes_in_task_t
                )

            x_test_t, y_test_t = extract_samples_according_to_labels(
                x_test, y_test, classes_in_task_t
            )
            ...
            if load_subject:
                ...
                train_data, val_data = make_valid_from_train_with_sub(
                    train_data, cut=cut
                )
            else:
        ...
        print(
            "Training set size: {}; Val set size: {}; Test set size: {}".format(
    ...
    def setup_offline(self, cut=0.9):
        ...
        x_train, y_train = extract_samples_according_to_labels(
            x_train, y_train, self.order_list
        )
        x_test, y_test = extract_samples_according_to_labels(
            x_test, y_test, self.order_list
        )
...
def make_valid_from_train_with_sub(dataset, cut=0.9):
    ...
    for cls in set(y_t.tolist()):
        x_cls, y_cls, sub_cls = extract_samples_according_to_labels_with_sub(
            x_t, y_t, sub_t, [cls]
        )
        perm = torch.randperm(len(x_cls))
        ...
        x_val_cls, y_val_cls, sub_val_cls = (
            x_cls[split:],
            y_cls[split:],
            sub_cls[split:],
        )
