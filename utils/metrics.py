...
def compute_performance(end_task_acc_arr):
    ...
    t_coef = stats.t.ppf(
        (1 + 0.95) / 2, n_run - 1
    )  # t coefficient used to compute 95% CIs: mean +- t*SD, alpha/2 = 0.975
    ...
    avg_end_acc = (
        (np.mean(avg_acc_per_run), t_coef * sem(avg_acc_per_run))
        if n_run > 1
        else (np.mean(avg_acc_per_run),)
    )
    ...
    avg_end_fgt = (
        (np.mean(avg_fgt), t_coef * sem(avg_fgt)) if n_run > 1 else (np.mean(avg_fgt),)
    )

    # compute Avg ACC on old tasks of each task
    acc_per_run = np.sum(np.tril(end_task_acc_arr), axis=2) / (np.arange(n_tasks) + 1)
    avg_acc = (
        (np.mean(acc_per_run, axis=0), t_coef * sem(acc_per_run, axis=0))
        if n_run > 1
        else (np.mean(acc_per_run, axis=0),)
    )
    ...
    avg_bwtp = (
        (np.mean(bwtp_per_run), t_coef * sem(bwtp_per_run))
        if n_run > 1
        else (np.mean(bwtp_per_run),)
    )
    ...
    avg_cur_acc = (
        (np.mean(diagonal_means), t_coef * sem(diagonal_means))
        if n_run > 1
        else (np.mean(diagonal_means),)
    )
