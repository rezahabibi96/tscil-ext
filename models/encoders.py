...
class CNNEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.in_channels = [
            2 ** (i - 1) * hidden_dims if i != 0 else input_channels
            for i in range(depth)
        ]

        for i in range(depth):
            if i == 0:
                stack = CNNEncoder.convStack(
                    self.in_channels[i], hidden_dims, norm, dropout
                )
            elif i == depth - 1:
                stack = CNNEncoder.convStack(
                    self.in_channels[i], feature_dims, norm, dropout
                )
            else:
                stack = CNNEncoder.convStack(
                    self.in_channels[i], 2 * self.in_channels[i], norm, dropout
                )
    ...
    def convStack(in_channels, out_channel, norm, dropout):
        ...
        stack = nn.Sequential(
            ...
            ),  # After each stack, the length will be halved.
            nn.Dropout(dropout),
        )
...
class TSTEncoder(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Encoder
        self.encoder = TSTEncoderLayerStack(
            seq_len,
            d_model,
            n_heads,
            d_k=d_k,
    ...
    def forward(self, x, pooling=True) -> Tensor:
        ...
        x = self.W_P(
            x
        )  # Revise W_p to fit 3d tensor                     # x: [bs x seq_len x d_model]
...
class TSTEncoderLayerStack(nn.Module):
    # used in TSTEncoder and TSTiEncoder
    def __init__(
        self,
        q_len,
        d_model,
...
#     # used in TCNEncoder
#     def __init__(
#         self, in_channels, channels, kernel_size, norm, dropout, extract_layers=None
#     ):
#         super().__init__()
